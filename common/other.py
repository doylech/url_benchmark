...
def static_scan(fn, inputs, start, reverse=False):
  """
  Sequentially apply fn and append result to 'outputs'.
  Inputs is a list with an auxiliary input to fn()
  for each step.
  :param fn: fn(prev, input_for_this_step)
  :param inputs: [nsteps x ...]
  ...
  indices = range(tf.nest.flatten(inputs)[0].shape[0]) # Does this output right number?
  if reverse:
    indices = reversed(indices)
  for index in indices:
    inp = tf.nest.map_structure(lambda x: x[index], inputs) # Get index'th value from inputs (copy instead of reference)?
    last = fn(last, inp)
