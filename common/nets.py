...
import tensorflow.keras.mixed_precision as prec # TODO: TF version change
...
class RSSM(common.Module):
  ...
  @tf.function
  def observe(self, embed, action, state=None, sample=True):
  ...
  def imagine(self, action, state=None, sample=True):
    ...
    action = swap(action)
    fn = functools.partial(self.img_step, sample=sample)
    prior = common.static_scan(fn, action, state)
...
class ConvEncoderMLP(common.Module):

  def __init__(
      self, depth=32, act=tf.nn.elu, kernels=(4, 4, 4, 4), keys=['image'], outputsize=512):
    self._act = getattr(tf.nn, act) if isinstance(act, str) else act
    self._depth = depth
  ...
  def __call__(self, obs):
    if tuple(self._keys) == ('image',):
      ...
      for i, kernel in enumerate(self._kernels):
        ...
        x = self._act(self.get(f'h{i}', tfkl.Conv2D, depth, kernel, 2)(x))  # stride = 2, so decreases size at each step
      x = tf.reshape(x, [x.shape[0], np.prod(x.shape[1:])])
      x = self.get(f'hMLP', tfkl.Dense, self._outputsize)(x) # No activation function on MLP output layer.
...
class MLP_deterministic(common.Module):
  ...
  def __call__(self, features):
    ...
    for index in range(self._layers-1):
      x = self.get(f'h{index}', tfkl.Dense, self._units, self._act)(x)
    x = self.get(f'h{index+1}', tfkl.Dense, self._units)(x) # No activation on the last layer.
    return x
...
class DistLayer(common.Module):
  ...
  def __call__(self, inputs):
    ...
    if self._dist == 'onehot':
      ...
      # return tfd.Independent(common.OneHotDist(out), len(self._shape)) #  Linqi suggested this - is that better?
