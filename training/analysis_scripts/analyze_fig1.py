...
def main():
  ...
  all_ids['random'] = {  # random policy playground
    'DRE-376': 't4-tf-2',
    'DRE-375': 't4-tf-1',
    'DRE-378': 't4-tf-2',
    'DRE-377': 't4-tf-1',
  ...
  expts_avg_collisions = {}
  for expt_set in expt_sets:
  ...
  # for expt_set, ids in all_ids.items():
    ...
    for env_num in env_nums:
      ...
      for id in all_df['id'].unique():
        ...
        for b in ball_ids:
          try:
            ...
            pass
        plt.axis('off')
        plt.title(f'{id}: {xlim*saverate/1e6} M steps')
        plt.show()
      ...
      expts_avg_collisions[expt_set] = all_avg_collisions
  ...
  legend_p = []
  for expt_set in expt_sets:
    avg_collisions = expts_avg_collisions[expt_set]
    for b in avg_collisions.keys():
      if len(avg_collisions[b]) > 0:
        avg_collisions[b] = np.vstack(avg_collisions[b])
        mm = np.mean(avg_collisions[b], axis=0)
        ss = scipy.stats.sem(avg_collisions[b], axis=0)
        tt = saverate * np.arange(len(mm))
        p, = plt.plot(tt, mm, color=expt_colors[expt_set])
        legend_p.append(p)
        plt.fill_between(tt, mm - ss, mm + ss, alpha=0.5, color=expt_colors[expt_set])
        legend_str.append(expt_set)
  plt.xlabel('Steps in env')
  plt.ylabel(f'Collision rate ({w_k}k step moving average)')
  plt.legend(legend_p, legend_str, frameon=False)
  simple_plot(plt.gca())
  plt.tight_layout()
  plot_dir = f"/home/saal2/logs/plots"
  plt.savefig(os.path.join(plot_dir, f'collision_rate_{expt_sets}.pdf'))
  plt.show()
