...
#
#
# def load_agent(basedir, env_num):
#   """Load a checkpointed Dreamer agent.
#   args:
#     basedir: str. i.e. /home/saal2/logs/GEN-304
#     env_num: int. which environment number (i.e. from multi-environment playgrounds)
#   """
#
...
if __name__ == "__main__":
  ...
  parser.add_argument('--expid', dest='expid', default='GEN-308',
                      help='experiment id (i.e. GEN-308)')
  parser.add_argument('--neps', dest='neps', default='60',
                      help='number of episodes to save into dataset')
  ...
  agnt = lu.load_agent(basedir, env_num)
  ...
  for which_ep in range(0, neps, 1):
    print(f'Ep: {which_ep}')
    ...
    df = pd.read_csv(fn)
    t = lu.get_closest_timestamp(df, traj_timestamp)
    ...
    if do_gif_recon:
      ...
      clip.write_gif(f'{basedir}/recon{which_ep}.gif', fps=20)

    # TODO: Make a dataset of position vs. state (or vs embedding?) for a trajectory
    all_xyz.append(xyz[:nt]) # XYZ coordinate of the agent
    all_logit.append(states['logit'][0]) # Probabilities for sampling categorical latent state
    all_deter.append(states['deter'][0]) # Deterministic hidden state of dynamics RNN
    all_which_ep.append(which_ep*np.ones(xyz[:nt, 0].shape))
  ...
  all_logit = np.vstack(all_logit)
  all_deter = np.vstack(all_deter)
  all_which_ep = np.vstack(all_which_ep)

  np.savez_compressed(f'{basedir}/vars_for_decode.npz',
                      xyz=all_xyz, logit=all_logit, deter=all_deter,
                      which_ep=all_which_ep)
