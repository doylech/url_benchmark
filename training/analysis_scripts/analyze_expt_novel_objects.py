...
def main():
  ...
  # xlim = int(250e3 / saverate)
  xlim = int(1.5e6 / saverate)
  ...
  for env_num in env_nums:
    ...
    for id in all_df['id'].unique():
      ...
      for b in ball_ids:
        try:
          tt = saverate * np.arange(np.cumsum(plot_df[f'{b}_yloc'])[:xlim].shape[0])
          p1, = plt.plot(tt, plot_df[f'{b}_yloc'][:xlim], '.',
                   markersize=2, color=colors[b], alpha=0.5)
          legend_str.append(f'{b}:' + labels[f'{b}'])
          legend_plots.append(p1)
          d = np.diff(plot_df[f'{b}_yloc'][:xlim])
          ii = np.argsort(d)
          appears[b] = np.minimum(ii[0], ii[1])
          exits[b] = np.maximum(ii[0], ii[1])
          plt.axvline(tt[appears[b]], linestyle='-', color=colors[b])
          plt.axvline(tt[exits[b]], linestyle='--', color=colors[b])
        except:
          pass
      plt.legend(legend_plots, legend_str, bbox_to_anchor=[1.05, 1])
