"""Load in a WM checkpoint and compare with corresponding full agent ckpt."""
...
if __name__ == "__main__":
  ...
  checkpoint_name = 'variables_train_agent_envindex0_010002500.pkl'
  # checkpoint_name = 'variables_train_agent_envindex0_007502500.pkl'
  wm_checkpoint = 'variables_train_agent_envindex0_010002500_wm.pkl'
  ...
  for i in range(len(agnt.variables)):
    print(i)
    assert(np.all(agnt.variables[i].numpy() == agnt0.variables[i].numpy()))

  for i in range(len(agnt.wm.variables)):
    print(i)
    assert(np.all(agnt.wm.variables[i].numpy() == agnt0.wm.variables[i].numpy()))

  for i in range(len(agnt._expl_behavior.variables)):
    print(i)
    assert(np.all(agnt._expl_behavior.variables[i].numpy() == agnt0._expl_behavior.variables[i].numpy()))
