...
def multiagent_novel_objects_step1_objects(agent, scale, environment_kwargs, include_tracker=True):
  """Arena for novel object"""
  m, v, h, w = custom_mazes.get_custom_maze(id='novel_objects')
  primary_agent = '0'
  agents = {}
  ...
  agents['3'] = jumping_ball.RollingBall(name="object3", size=[1.1 * scale, 1.1 * scale, 1.1 * scale], rgb1=[1.0, 1., 1.],  rgb2=[0.8, 0.8, 0.8], mass=2, friction=1.)
  agents['4'] = jumping_ball.RollingBall(name="object4", size=[1.1 * scale, 1.1 * scale, 1.1 * scale], rgb1=[1.0, 1., 1.],  rgb2=[0.8, 0.8, 0.8], mass=0.1, friction=1.)
...
def multiagent_interactive_flashing_object(agent, scale, environment_kwargs):
  ...
  agents = {}
  agents[primary_agent] = agent
  agents['1'] = jumping_ball.RollingBall(name="object1", size=[1.1 * scale, 1.1 * scale, 1.1 * scale], rgb1=[1., 1., 1.],  rgb2=[0.8, 0.8, 0.8], mass=1, friction=1.)
  agents['2'] = jumping_ball.RollingBall(name="object2", size=[1.1 * scale, 1.1 * scale, 1.1 * scale], rgb1=[1., 1., 1.],  rgb2=[0.8, 0.8, 0.8], mass=1, friction=1.)
  # agents['3'] = jumping_ball.RollingBall(name="object3", size=[1.1 * scale, 1.1 * scale, 1.1 * scale], rgb1=[1., 1., 1.],  rgb2=[0.8, 0.8, 0.8], mass=1, friction=1.)
  ...
  env.policies = {'object1': functools.partial(pol.object_appear, walker=agents['1'], step_appear=0*fs, step_disappear=None, start_pos=[5*s, (3-2)*s]),
                  'object2': functools.partial(pol.object_appear_and_color_change_collision, walker=agents['2'],
                                               step_appear=0*fs,
                                               step_disappear=None, start_pos=[5*s, (-3-2)*s], do_random=True,
